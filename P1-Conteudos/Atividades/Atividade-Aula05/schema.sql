...
SELECT e.titulo,
       ...
       l.cidade    AS cidade
FROM evento e
JOIN tipo_evento t  ON e.idtipoevento  = t.idtipoevento
...
SELECT titulo, descricao, dataHora, status
FROM evento
WHERE status = 'Ativo';


SELECT mensagem, dataHora, nivel
FROM alerta
WHERE nivel = 'Alto';
...
SELECT e.titulo, e.dataHora, l.cidade
FROM evento e
JOIN localizacao l ON e.idlocalizacao = l.idlocalizacao
WHERE l.cidade = 'São Paulo';

SELECT r.texto, r.dataHora, u.nome AS usuario
FROM relato r
JOIN usuario u ON r.idusuario = u.idusuario
WHERE r.dataHora > '2025-09-01 00:00:00';

SELECT h.id_evento, h.status_anterior, h.status_novo, h.data_alteracao
FROM historico_evento h
WHERE h.status_novo = 'Arquivado';
